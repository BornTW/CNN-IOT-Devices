...
# X_train [99598,25] X_test [33200,25] Y_train [99598,25] Y_test [33200,25]
...
def train():
    ...
    for epoch in range(1, epochs + 1):
        ...
        for X_batch, y_batch in dataloader:
            ...
            X_batch = X_batch.reshape(-1, 1, 5, 5)
        ...
        print(message)
        with open("cnn.txt", "a") as log_file:
...
def test():
    ...
    model = CnnNet().cuda()
    model.load_state_dict(torch.load("./cnn_71.70.pth"))
    model.eval()

    utils.visual_model(model, [1, 1, 5, 5])
